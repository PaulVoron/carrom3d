...
  subStepping: 2,         // Вычислять физику 120 раз в сек (2 * 60fps)
};
...
// --- ВИЗУАЛЬНЫЙ ДЕБАГГИНГ ---
function drawDebugPockets() {
  // Столб высотой 50 сантиметров! 
  const geo = new THREE.CylinderGeometry(PHYSICS.pocketRadius, PHYSICS.pocketRadius, 0.5, 32);
  // Яркий, непрозрачный зеленый цвет
  const mat = new THREE.MeshBasicMaterial({ 
    color: 0x00ff00, 
    wireframe: true // Делаем сеткой, чтобы точно видеть сквозь них
  });

  console.log("📍 Координаты луз (pocketCenters):", pocketCenters);

  pocketCenters.forEach((pos, index) => {
    const mesh = new THREE.Mesh(geo, mat);
    // Поднимаем столб высоко вверх, чтобы он торчал над доской
    mesh.position.set(pos.x, 0.25, pos.z);
    scene.add(mesh);
    console.log(`Луза ${index + 1}: X=${pos.x.toFixed(3)}, Z=${pos.z.toFixed(3)}`);
  });
}
...
  // Нам нужна ссылка на тело невидимого пола, чтобы отключать коллизии
  // (Предполагается, что оно лежит в world, но проще фильтровать по группам. 
  // В Rapier можно отключить коллизии, сделав тело "сенсором")
  
  physicsBodies.forEach((entry) => {
    const { mesh, body } = entry;
    
    // Проверяем только активные динамические объекты на столе
    if (!body.isEnabled() || body.bodyType() !== RAPIER.RigidBodyType.Dynamic) return;

    const pos = body.translation();
    const vecPos = new THREE.Vector3(pos.x, 0, pos.z); // Плоскость XZ
    
    // 1. Проверяем балансирование над лузой
    let isOverPocket = false;
    for (const pocketCenter of pocketCenters) {
      const dist = vecPos.distanceTo(pocketCenter);
      // Если центр фишки зашел за радиус лузы — она падает!
      if (dist < PHYSICS.pocketRadius * 0.9) { 
        isOverPocket = true;
        break;
      }
    }
...
    if (isOverPocket) {
      // МАГИЯ ПАДЕНИЯ: Превращаем коллайдер фишки в сенсор
      // Сенсоры подчиняются гравитации, но НЕ сталкиваются с полом и крышкой!
      // Фишка проваливается сквозь стол, сохраняя скорость X/Z.
      if (!fallingQueue.has(body.handle)) {
        const collider = world.getCollider(body.collider(0));
        if (collider) collider.setSensor(true);
        fallingQueue.set(body.handle, true);
        console.log(`🕳️ Падение началось: ${mesh.userData.type}`);
      }
    }

    // 2. Окончательное удаление фишки (когда упала глубоко в лузу)
    if (fallingQueue.has(body.handle) && pos.y < PHYSICS.pocketFallDepth) {
...
  const halfSize = 0.735 / 2; // Половина ширины игровой зоны стола
  const offset = halfSize - PHYSICS.pocketRadius;

  // Идеальные математические центры 4-х луз
  pocketCenters = [
...
  ];
  
  // Мы больше не создаем колайдеры (сенсоры) луз! 
  // Мы будем вычислять падение математически в главном цикле.
}
...
  body.setLinvel({ x: 0, y: 0, z: 0 }, true);
  body.sleep();
  body.setEnabled(false); // Полностью выключаем из симуляции
  mesh.visible = false;
...
      // 1. Измеряем точные габариты деревянной текстуры
      const bbox = new THREE.Box3().setFromObject(obj);
      boardTopY = 0.002; // Твой хак высоты оставляем

      // === АВТО-ОПРЕДЕЛЕНИЕ ЛУЗ ===
      // Измеряем ширину и глубину игрового поля
      const minX = bbox.min.x;
      const maxX = bbox.max.x;
      const minZ = bbox.min.z;
      const maxZ = bbox.max.z;

      // Лузы находятся в углах. Немного отступаем от самого края внутрь (на радиус лузы)
      const pr = PHYSICS.pocketRadius;
      pocketCenters = [
        new THREE.Vector3(maxX - pr, 0, maxZ - pr), // Правый нижний
        new THREE.Vector3(maxX - pr, 0, minZ + pr), // Правый верхний
        new THREE.Vector3(minX + pr, 0, maxZ - pr), // Левый нижний
        new THREE.Vector3(minX + pr, 0, minZ + pr)  // Левый верхний
      ];
      // ==============================
...
      // НЕВИДИМАЯ КРЫШКА (чтобы фишки не улетали со стола)
      const roofHeight = 0.04; // 4 см над столом
